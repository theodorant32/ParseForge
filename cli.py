"""
cli.py — ParseForge Command-Line Interface

Commands:
  run    — run pipeline on a single input (or stdin with "-")
  batch  — run pipeline on all inputs in a JSON fixture file
  serve  — start the FastAPI server

Examples:
  python cli.py run "Find me 2 people for a robotics project this weekend"
  echo "I need help with calculus" | python cli.py run -
  python cli.py run "urgent gig for a designer" --skip-enrichment --output json
...
# Resolve package root so we can import parseforge regardless of cwd
...
def run(text: str, skip_enrichment: bool, output: str, verbose: bool, save: bool, trace_id: str | None):
    """Run the pipeline on TEXT. Use \"-\" to read from stdin."""
    import logging
...
def batch(fixture_file: str, output: str, save: bool, skip_enrichment: bool):
    """Run the pipeline on all inputs in FIXTURE_FILE (JSON array or object with 'inputs' key)."""
    import logging
    ...
    # Support both flat array and {"inputs": [...]} format
    ...
    for i, case in enumerate(cases, 1):
        # Support both plain strings and {"input": ..., "expected_action": ...} dicts
...
def interactive(skip_enrichment: bool, verbose: bool):
    """Start an interactive REPL to test the pipeline."""
    import logging
    logging.getLogger().setLevel(logging.ERROR if not verbose else logging.DEBUG)

    click.echo(_c("🏦 Welcome to the ParseForge Trading Floor (Interactive Mode)", Fore.MAGENTA + Style.BRIGHT))
...
def serve(host: str, port: int, reload: bool):
    """Start the ParseForge FastAPI server."""
    try:
        import uvicorn
    except ImportError:
